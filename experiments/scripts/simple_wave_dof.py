...
class ProblemDefinition:
    ...
    def solution_u(self, t: jnp.ndarray, x: jnp.ndarray) -> jnp.ndarray:
        ...
        is_boundary = jnp.isclose(x_array, self.x_min) | jnp.isclose(
            x_array, self.x_max
        )
    ...
    def solution_v(self, t: jnp.ndarray, x: jnp.ndarray) -> jnp.ndarray:
        ...
        return (
            (jnp.pi / self.T)
            * jnp.cos(jnp.pi * t / self.T)
            * jnp.sin(jnp.pi * (x - self.x_min) / (self.x_max - self.x_min))
        )
    ...
    def source_f(self, t: jnp.ndarray, x: jnp.ndarray) -> jnp.ndarray:
        ...
        coeff = (
            -jnp.pi**2 / self.T**2
            + self.c**2 * jnp.pi**2 / (self.x_max - self.x_min) ** 2
        )
        return coeff * u
    ...
    def _plot_dof_metrics(
        ...
        if mode == "loss" and show_fgk_results:
            if self.problem.cfg["integration"]["spatial_dim"] == 1:
                ...
                plt.plot(
                    [r[0] for r in results_p1],
                    [r[1] for r in results_p1],
                    "s-",
                    label="p = 1, FGK23",
                )
                ...
                plt.plot(
                    [r[0] for r in results_p2],
                    [r[1] for r in results_p2],
                    "s-",
                    label="p = 2, FGK23",
                )
            else:
